---
name: stallning-testing
description: Write tests in the Stallning monorepo including unit tests, component tests, visual regression tests, and mutation testing with Stryker. Use when creating tests, fixing test failures, adding test coverage, or running mutation testing.
---
...
For screenshot comparisons in a real browser. Runs with Playwright.

Use `expectPngSnapshot` (`packages/ui/test/visual/png-snapshot.ts`) so baselines are **binary PNGs**, not base64 text mistaken for images. On `vitest -u`, Playwright writes the file; otherwise base64 from the browser is compared to `readFile(..., { encoding: 'base64' })` on the server.

**Key rules:**

- Prefer real SFCs + Tailwind (`packages/ui/test/visual/`)
- Baselines live in `test/**/__screenshots__/<spec-file>/` — commit them
- One test per visual state (default, hover, disabled, each intent)
- Run `pnpm test:visual -- --update` to regenerate baselines

## Mutation Testing (Stryker)

Validates test quality by introducing mutations (changing `>` to `>=`, removing lines, etc.) and verifying tests catch them.
